...
// conditionMatchesValue delegates to config.ConditionMatchesValue.
// It is called when condition.Matches is nil (Datatype was not specified).
func conditionMatchesValue(condition *config.RulesBasedSamplerCondition, value interface{}, exists bool) bool {
	return config.ConditionMatchesValue(condition, value, exists)
}
